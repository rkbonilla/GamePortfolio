...
for (var i = 0; i < amount; i++) {
	particles[i] = new GameObject({ width: 0, height: 0, x: platform[1].x, y: platform[1].y, color: "#ffff00" });
}
//---------------------------------------------------------------------------------------------------------------
...
function animate() {
	...
	if (player.hitTestObject(platform[1])) {
		...
		boomTimer = setTimeout(boom, 1000);
	}
	//----------------------------------------------------------------------------------------------------------------------
	...
	for (var p = 0; p < particles.length; p++) {
		...
		context.drawImage(img, particles[p].x -50, particles[p].y-50, particles[p].width * 10, particles[p].height * 10);
	}
...
function boom() {
	...
	for (var i = 0; i < particles.length; i++) {
		platform[1].y = -100000;
		particles[i].width = 10;
		particles[i].height = 10;
		particles[i].vx = randomRange(10, -10);
		particles[i].vy = randomRange(10, -10);
	}
}
